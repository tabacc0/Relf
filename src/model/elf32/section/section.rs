...
impl<'a> Elf32Section<'a> {
    ...
    fn calc_symbol_offset(&self, idx: usize) -> Result<Elf32Off, Error> {
        let symbol_entry_size = ELF32SYMSIZE;
        let entries_number =
            u32::from(self.header.sh_size()) as usize / symbol_entry_size;
    ...
    pub fn symbol(&'a self, idx: usize) -> Result<Elf32Symbol<'a>, Error> {
    ...
    fn calc_rel_offset(&self, idx: usize) -> Result<Elf32Off, Error> {
        let entries_number =
            u32::from(self.header.sh_size()) as usize / ELF32RELSIZE;
    ...
    fn calc_rela_offset(&self, idx: usize) -> Result<Elf32Off, Error> {
        let rela_entry_size = ELF32RELASIZE;
        let entries_number =
            u32::from(self.header.sh_size()) as usize / rela_entry_size;
    ...
    pub fn rela(&self, idx: usize) -> Result<Elf32Rela, Error> {
        ...
        let rela = match Elf32Rela::from_bytes(rela_bytes, self.endianness)
        {
            Ok(value) => value,
    ...
    pub fn relocation_iter(&'a self) -> Elf32RelocationIter<'a> {
        Elf32RelocationIter::new(&self)
    }
}
