...
fn main() -> Result<(), Error> {
    let elf_bytes = match std::fs::read("./test-binaries/nm-test-64bits") {
        Ok(f) => f,
        Err(e) => {
            println!("{}", e);
            return Ok(());
    ...
    for symbol in symbol_arr {
        ...
        let related_section = match elf_file.section(symbol.section_idx())
        {
            Ok(section) => section,
        ...
        symbol_type = match true {
            ...
            val if val == related_section.is_nobits() => "b".to_string(),
            val if val != related_section.is_writable() => "r".to_string(),
